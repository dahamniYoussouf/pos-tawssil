...
class LocationService {
  ...
  static Future<Position?> getCurrentLocation() async {
    try {
      ...
      print(
          "Coordonnées obtenues - Latitude: ${position.latitude}, Longitude: ${position.longitude}");
      return position;
    } catch (e) {
  ...
  static Future<bool> sendGpsLocationToServer(
      double latitude, double longitude) async {
    try {
  ...
  static Future<bool> sendLocationWithCoordinates(
    ...
    try {
      ...
      try {
        ...
        if (locations.isNotEmpty) {
          ...
          print(
              "Géocodage réussi: ${location.latitude}, ${location.longitude}");

          return await sendLocationWithCoordinates(
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return await sendLocationWithCoordinates(
            address, 36.7538, 3.0588); // Coordonnées d'Alger
      } else {
        print(
            "ÉCHEC adresse: Erreur serveur avec le code ${response.statusCode}");
        return false;
      }
    } catch (e) {
