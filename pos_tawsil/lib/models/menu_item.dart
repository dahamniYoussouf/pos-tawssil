...
class MenuItem {
  ...
  factory MenuItem.fromJson(Map<String, dynamic> json) {
    return MenuItem(
      ...
      photoUrl: json['photo_url'] ?? json['image_url'] ?? json['image'] ?? json['photo'],
  ...
  static List<MenuItemPromotion> _decodePromotions(dynamic raw) {
    ...
    try {
      ...
      return decoded.map((entry) {
        ...
        if (entry is Map) {
          return MenuItemPromotion.fromMap(Map<String, dynamic>.from(entry));
        }
        return null;
      }).whereType<MenuItemPromotion>().toList();
    } catch (_) {
      return const [];
    }
  }

  static List<MenuItemPromotion> _extractPromotionsFromApi(Map<String, dynamic> json) {
      ...
      if (source is Iterable) {
        for (final item in source) {
          if (item is Map<String, dynamic>) {
            ...
            final promo = MenuItemPromotion.fromJson(Map<String, dynamic>.from(item));
            if (promo.id.isNotEmpty) {
              merged[promo.id] = promo;
            }
          }
