...
class DatabaseConfig {
  ...
  static Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      await db.execute("ALTER TABLE menu_items ADD COLUMN promotions_json TEXT");
    }

    if (oldVersion < 6) {
      await db.execute("ALTER TABLE order_items ADD COLUMN promotion_price REAL");
    }
    ...
    if (oldVersion < 7) {
      ...
      ''');
      try {
        await db.execute("ALTER TABLE additions ADD COLUMN option_group_id TEXT");
      } catch (_) {}
    }
    ...
    if (oldVersion < 9) {
      try {
        await db.execute('ALTER TABLE print_logs ADD COLUMN order_number TEXT');
      } catch (_) {}
      try {
        await db.execute('ALTER TABLE print_logs ADD COLUMN device_key TEXT');
      } catch (_) {}
    }
  }
  ...
  ''');
    ...
    // Print Logs (for offline print tracking)
    await db.execute('''
