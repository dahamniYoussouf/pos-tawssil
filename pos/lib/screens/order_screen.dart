...
class _OrderScreenState extends State<OrderScreen> {
  ...
  MenuItem? _findLoadedMenuItem(String menuItemId) {
  ...
  Future<MenuItem?> _resolveMenuItemForOrderItem(OrderItem item) async {
    final loaded = _findLoadedMenuItem(item.menuItemId);
    if (loaded != null) {
      return loaded;
    }
    return _db.getMenuItemById(item.menuItemId);
  }

  bool _canEditOrderItem(OrderItem item) {
    final menuItem = _findLoadedMenuItem(item.menuItemId);
    if (menuItem == null) {
      return item.additions.isNotEmpty;
    }
    return menuItem.optionGroups.isNotEmpty ||
        menuItem.additions.any((add) => add.isAvailable) ||
        item.additions.isNotEmpty;
  }

  Future<void> _editOrderItem(OrderItem item) async {
    final menuItem = await _resolveMenuItemForOrderItem(item);
    if (!mounted) return;

    if (menuItem == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Article introuvable pour modification'),
    ...
    final selection = await _showAdditionsSheet(
      context,
      menuItem,
      initialQuantity: item.quantite,
      initialAdditions: item.additions,
      submitLabel: 'Mettre à jour',
    );

    if (!mounted || selection == null) return;

    context.read<OrderProvider>().updateItemConfiguration(
          item.id,
          menuItem,
          quantity: selection.quantity,
          specialInstructions: item.instructionsSpeciales,
...
class _OrderSummaryState extends State<_OrderSummary> {
  ...
  Future<void> _editOrderItem(OrderItem item) async {
    final menuItem = await _db.getMenuItemById(item.menuItemId);
    if (!mounted) return;
                    ...
                    builder: (context) {
                      ...
                      return ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final editItem = () => _editOrderItem(item);
                          return Dismissible(
                            key: ValueKey(
                              'order-item-${item.id}-${item.createdAt.toIso8601String()}',
                            ),
                            ...
                            background: Container(
                              decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(10),
                                border: Border.all(
                                  color: TawsilColors.error.withOpacity(0.28),
                                ),
                              ),
                            ...
                            secondaryBackground: Container(
                              decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(10),
                                border: Border.all(
                                  color: TawsilColors.primary.withOpacity(0.2),
                                ),
                              ),
                          ...
                          SizedBox(
                            ...
                            child: OutlinedButton(
                              ...
                              style: OutlinedButton.styleFrom(
                                ...
                                side: BorderSide(
                                  color: TawsilColors.primary.withOpacity(0.22),
                                ),
                                shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(
                                    TawsilBorderRadius.full,
...
Future<_AdditionSelection?> _showAdditionsSheet(
  ...
  List<OrderItemAddition> initialAdditions = const [],
  String submitLabel = 'Ajouter au panier',
}) async {
  final availableAdditions =
      menuItem.additions.where((a) => a.isAvailable).toList();
  final groups = <OptionGroup>[];
...
class _AdditionsSheetDialog extends StatefulWidget {
  ...
  final List<OrderItemAddition> initialAdditions;
  ...
  const _AdditionsSheetDialog({
    ...
    this.initialAdditions = const [],
    this.submitLabel = 'Ajouter au panier',
  });
...
class _AdditionsSheetDialogState extends State<_AdditionsSheetDialog> {
  ...
  void initState() {
    ...
    _itemQty = widget.initialQuantity > 0 ? widget.initialQuantity : 1;
    for (final addition in widget.initialAdditions) {
      if (_selected.containsKey(addition.additionId)) {
        _selected[addition.additionId] = addition.quantity > 0 ? addition.quantity : 0;
      }
    }
  }
  ...
  Widget _buildHeaderChip(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          if (icon != null) ...[
            Icon(icon, size: 11, color: resolvedTextColor),
            const SizedBox(width: 3),
          ],
          Text(
            label,
  ...
  Widget _buildGroupBadge(OptionGroup group) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(TawsilBorderRadius.full),
        border: Border.all(
          color: TawsilColors.error.withOpacity(0.24),
        ),
      ),
      child: Text(
        'REQUIS',
  ...
  Widget _buildQuantitySection({required bool compact}) {
    final buttonSize = compact ? 34.0 : 50.0;
