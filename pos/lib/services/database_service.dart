...
class DatabaseService {
  ...
  Future<MenuItem> _hydrateMenuItem(Map<String, dynamic> map) async {
    final additions = await _getAdditionsForMenuItem(map['id']);
    final optionGroups =
        await _getOptionGroupsForMenuItem(map['id'], additions);
    return MenuItem.fromMap(map)
        .copyWith(additions: additions, optionGroups: optionGroups);
  }

  Future<List<MenuItem>> getMenuItems() async {
    final db = await DatabaseConfig.database;
    final List<Map<String, dynamic>> maps =
        await db.query('menu_items', orderBy: 'nom COLLATE NOCASE ASC');
    return Future.wait(maps.map(_hydrateMenuItem));
  }
  ...
  Future<List<MenuItem>> getMenuItemsForRestaurant(String restaurantId) async {
    ...
    final maps = await db.rawQuery(
      ...
      WHERE c.restaurant_id = ? OR c.id IS NULL
      ORDER BY
        CASE WHEN c.ordre_affichage IS NULL THEN 1 ELSE 0 END ASC,
        c.ordre_affichage ASC,
        m.nom COLLATE NOCASE ASC
      ''',
      [restaurantId],
    );
    return Future.wait(maps.map(_hydrateMenuItem));
  ...
  Future<MenuItem?> getMenuItemById(String id) async {
    ...
    return _hydrateMenuItem(maps.first);
  }
  ...
  Future<void> clearMenuCatalog() async {
    final db = await DatabaseConfig.database;
    await db.transaction((txn) async {
      await txn.delete('additions');
      await txn.delete('option_groups');
      await txn.delete('menu_items');
      await txn.delete('food_categories');
    });
  }
  ...
  Future<void> insertMenuItem(MenuItem item) async {
    ...
    for (final addition
        in item.additions.where((a) => !groupedAdditionIds.contains(a.id))) {
      final additionMap = addition.toMap();
      additionMap['menu_item_id'] =
          item.id; // ensure linkage even if API omits it
      await db.insert(
        'additions',
        additionMap,
        conflictAlgorithm: ConflictAlgorithm.replace,
  ...
  Future<void> updateMenuItem(MenuItem item) async {
    ...
    for (final addition
      ...
      final additionMap = addition.toMap();
      additionMap['menu_item_id'] =
          item.id; // ensure linkage even if missing in payload
      await db.insert(
        'additions',
        additionMap,
        conflictAlgorithm: ConflictAlgorithm.replace,
  ...
  Future<List<Map<String, dynamic>>> getFoodCategories(
      {String? restaurantId}) async {
    final db = await DatabaseConfig.database;
    return await db.query(
      'food_categories',
      where: restaurantId == null ? null : 'restaurant_id = ?',
      whereArgs: restaurantId == null ? null : [restaurantId],
  ...
  Future<void> markOrderAsSyncedWithServer({
    ...
    await db.transaction((txn) async {
      final existingServerOrder = await txn.query(
        'orders',
        columns: ['id'],
        where: 'id = ?',
      ...
      if (existingServerOrder.isNotEmpty && serverOrderId != localOrderId) {
        await txn.delete(
          'print_logs',
          where: 'order_id = ?',
...
// ==================== GET ORDERS STATISTICS ====================
  ...
  Future<Map<String, dynamic>> getOrdersStatistics() async {
    ...
    final revenueAllResult =
        await db.rawQuery('SELECT SUM(total_amount) as total FROM orders');
    final revenueSyncedResult = await db.rawQuery(
        'SELECT SUM(total_amount) as total FROM orders WHERE synced = 1');
    final totalRevenueAll =
        (revenueAllResult.first['total'] as num?)?.toDouble() ?? 0.0;
    final totalRevenueSynced =
        (revenueSyncedResult.first['total'] as num?)?.toDouble() ?? 0.0;
    ...
    final unsyncedResult = await db
        .rawQuery('SELECT COUNT(*) as count FROM orders WHERE synced = 0');
