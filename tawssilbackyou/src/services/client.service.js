...
export const getClientOrdersWithFilters = async (filters) => {
  ...
  const formattedOrders = rows.map(order => ({
    ...
    order_items: order.order_items.map((item) => {
      ...
      const menuUnitPrice = item.menu_item?.prix != null ? parseFloat(item.menu_item.prix) : null;
      const priceBefore = Number.isFinite(menuUnitPrice) ? menuUnitPrice : unitPrice;
      const priceAfter = Number.isFinite(unitPrice) ? unitPrice : 0;
      const promotionApplied = priceBefore - priceAfter > 0.01;
