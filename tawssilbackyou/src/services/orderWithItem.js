...
export async function createOrderWithItems(data) {
  ...
  const [restaurant, menuItems, additions, requiredOptionGroups, configuredPreparationTime, configuredDefaultDeliveryFee] = await Promise.all([
    Restaurant.findOne({
  ...
  const orderStatus = isPosOrder ? 'accepted' : 'pending';
  const acceptedAt = isPosOrder ? new Date() : null;
