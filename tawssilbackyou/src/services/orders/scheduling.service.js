...
const parseTimeoutMinutes = (value, fallback, min, max) => {
...
export function scheduleRestaurantPreparationTimeout(orderId) {
  void (async () => {
    let timeoutMinutes = 20;

    try {
      const configuredTimeout = await SystemConfig.get('restaurant_preparation_timeout', 20);
      timeoutMinutes = parseTimeoutMinutes(configuredTimeout, 20, 1, 180);
    } catch (error) {
      console.error("❌ Error reading restaurant_preparation_timeout config:", error);
    }
    ...
    setTimeout(async () => {
      try {
        ...
        if (order && ['accepted', 'preparing'].includes(order.status)) {
          const { createRestaurantPreparationTimeoutNotification } = await import("../adminNotification.service.js");
          await createRestaurantPreparationTimeoutNotification(orderId);
        }
      } catch (error) {
        console.error("❌ Error scheduling restaurant preparation notification:", error);
      }
    }, timeoutMinutes * 60 * 1000);
  })();
}

export function scheduleDriverArrivalTimeout(orderId) {
  void (async () => {
    let timeoutMinutes = 15;

    try {
      const configuredTimeout = await SystemConfig.get('driver_arrival_timeout', 15);
      timeoutMinutes = parseTimeoutMinutes(configuredTimeout, 15, 1, 120);
    } catch (error) {
      console.error("❌ Error reading driver_arrival_timeout config:", error);
    }
    ...
    setTimeout(async () => {
      try {
        ...
        if (order && order.status === "assigned") {
          const { createDriverArrivalTimeoutNotification } = await import("../adminNotification.service.js");
          await createDriverArrivalTimeoutNotification(orderId);
        }
      } catch (error) {
        console.error("❌ Error scheduling driver arrival notification:", error);
      }
    }, timeoutMinutes * 60 * 1000);
  })();
}

export function scheduleDriverDeliveryTimeout(orderId) {
  void (async () => {
    let timeoutMinutes = 45;

    try {
      const configuredTimeout = await SystemConfig.get('driver_delivery_timeout', 45);
      timeoutMinutes = parseTimeoutMinutes(configuredTimeout, 45, 1, 240);
    } catch (error) {
      console.error("❌ Error reading driver_delivery_timeout config:", error);
    }
    ...
    setTimeout(async () => {
      try {
        ...
        if (order && order.status === "delivering") {
          const { createDriverDeliveryTimeoutNotification } = await import("../adminNotification.service.js");
          await createDriverDeliveryTimeoutNotification(orderId);
        }
      } catch (error) {
        console.error("❌ Error scheduling driver delivery notification:", error);
      }
    }, timeoutMinutes * 60 * 1000);
  })();
}
