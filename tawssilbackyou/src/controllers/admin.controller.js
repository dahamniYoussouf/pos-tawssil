...
import { getLatestDeviceTokenForProfile, notifyProfiles, notifyRole } from "../services/notification.service.js";
...
export const getAllConfigs = async (req, res, next) => {
  try {
    const cacheKey = 'admin:configs:all:v5';
...
export const notifyAllDrivers = async (req, res, next) => {
  try {
    ...
    try {
      await notifyProfiles('driver', driverIds, 'admin_broadcast', notificationPayload);
    } catch (notifyError) {
  ...
  try {
    ...
    for (const target of selectedTargets) {
      if (target === 'client') {
        ...
        try {
          await notifyRole('client', 'admin_broadcast', { ...basePayload, audience: 'clients' });
        } catch (notifyError) {
          console.error("Client broadcast failed:", notifyError);
        }
      }
      ...
      if (target === 'driver') {
        ...
        totalRecipients += count;
        try {
          await notifyRole('driver', 'admin_broadcast', { ...basePayload, audience: 'drivers' });
        } catch (notifyError) {
          console.error("Driver broadcast failed:", notifyError);
        }
      }
      ...
      if (target === 'restaurant') {
        ...
        totalRecipients += count;
        try {
          await notifyRole('restaurant', 'admin_broadcast', { ...basePayload, audience: 'restaurants' });
        } catch (notifyError) {
          console.error("Restaurant broadcast failed:", notifyError);
        }
      }
      ...
      if (target === 'admin') {
        ...
        totalRecipients += count;
        try {
          await notifyRole('admin', 'admin_broadcast', { ...basePayload, audience: 'admins' });
        } catch (notifyError) {
          console.error("Admin broadcast failed:", notifyError);
        }
      }
    }
