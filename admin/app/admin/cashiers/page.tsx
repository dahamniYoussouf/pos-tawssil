...
export default function CashierManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        const error: any = new Error(getApiErrorMessage(err, 'Echec de creation'));
      ...
      const fieldErrors = getApiValidationErrors(e);
      if (Object.keys(fieldErrors).length > 0) {
        setCreateFormErrors(fieldErrors);
      }
      setError(getApiErrorMessage(e, 'Erreur inconnue'));
    } finally {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(getApiErrorMessage(err, 'Echec de mise a jour'));
      }
