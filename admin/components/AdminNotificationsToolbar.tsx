...
> = {
  ...
  orders: {
    label: 'Commandes',
    icon: Clock,
    types: ['pending_order_timeout', 'order_admin_review_required']
  },
  restaurants: {
    label: 'Restaurants',
    icon: Store,
...
export default function AdminNotificationsToolbar() {
  ...
  const handleOpenDetails = (notificationId: string, type?: NotificationType) => {
    if (type === 'order_admin_review_required') {
            ...
            {!loading &&
              visibleList.map((notification) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => handleOpenDetails(notification.id, notification.type)}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter' || event.key === ' ') {
                      event.preventDefault();
                      handleOpenDetails(notification.id, notification.type);
                    }
                  }}
                >
