...
export const createOrderWithItemsValidator = [
  ...
  body('items.*.options.*.option_id')
    .optional()
    .isUUID().withMessage('option_id must be a valid UUID'),

  body('items.*.options.*.quantity')
    .optional()
    .isInt({ min: 1 }).withMessage('Option quantity must be at least 1'),
...
export const getAllOrdersValidator = [
  ...
  query('status')
    .optional()
    .isIn(['pending', 'pending_admin_review', 'accepted', 'preparing', 'assigned', 'arrived', 'delivering', 'delivered', 'declined'])
