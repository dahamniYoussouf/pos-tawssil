  ...
  io.on("connection", async (socket) => {
    ...
    try {
      ...
      if (socket.userRole === "cashier") {
        const cashier = await Cashier.findOne({ where: { user_id: socket.userId } });
        if (cashier) {
          socket.join(`cashier:${cashier.id}`);
          socket.join(`restaurant:${cashier.restaurant_id}`);
          socket.cashierProfileId = cashier.id;
          socket.restaurantProfileId = cashier.restaurant_id;
          console.log(`🧾 Cashier joined: cashier:${cashier.id} | restaurant:${cashier.restaurant_id}`);
        }
      }
