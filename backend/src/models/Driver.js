...
const Driver = sequelize.define('Driver', {
  ...
  locale: {
    ...
    comment: "Driver preferred locale (ar/fr/en)"
  },
  vehicle_type: {
  ...
  rating: {
    ...
    comment: "Average rating (0-5 stars), 0 means no ratings yet"
  },
  total_deliveries: {
...
const getActiveOrdersCount = (driver) => (
...
Driver.prototype.isAvailable = function() {
  return this.status === 'available' && 
         this.is_active && 
         this.is_verified &&
         getActiveOrdersCount(this) < this.max_orders_capacity;
};
...
Driver.prototype.canAcceptMoreOrders = function() {
  return ['available', 'busy'].includes(this.status) &&
         this.is_active &&
         this.is_verified && 
         getActiveOrdersCount(this) < this.max_orders_capacity;
};
...
Driver.prototype.hasActiveOrders = function() {
  return getActiveOrdersCount(this) > 0;
};
...
Driver.prototype.getActiveOrdersCount = function() {
  return getActiveOrdersCount(this);
};
