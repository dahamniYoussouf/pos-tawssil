...
import { getDateStampInAppTimeZone } from "../utils/appTime.js";
...
const Order = sequelize.define('Order', {
  ...
  status: {
    type: DataTypes.ENUM(
      ...
      "pending_admin_review", // Awaiting admin contact/approval
  ...
  requires_admin_review: {
    ...
    allowNull: false,
    defaultValue: false,
    comment: "Marks orders that must be reviewed by an admin before restaurant notification"
  },
  ...
  admin_review_status: {
    ...
    allowNull: false,
    defaultValue: "none",
    comment: "Admin review workflow state (none, pending, claimed, approved, rejected)"
  },

  admin_review_metadata: {
    type: DataTypes.JSONB,
    allowNull: true,
    comment: "Review routing metadata and thresholds used for this order"
  },

  admin_review_claimed_by: {
    type: DataTypes.UUID,
    allowNull: true,
    comment: "Admin currently handling the client contact"
  },

  admin_review_claimed_at: {
    type: DataTypes.DATE,
    allowNull: true,
    comment: "When the admin claimed the order review"
  },

  admin_reviewed_by: {
    type: DataTypes.UUID,
    allowNull: true,
    comment: "Admin who approved or rejected the review"
  },

  admin_reviewed_at: {
    type: DataTypes.DATE,
    allowNull: true,
    comment: "When the admin completed the review"
  },
  ...
  restaurant_notified_at: {
    type: DataTypes.DATE,
    allowNull: true,
    comment: "When the restaurant was notified after admin approval"
  },
...
Order.generateOrderNumber = async function(orderType = 'delivery') {
  const date = getDateStampInAppTimeZone(new Date()) || new Date().toISOString().slice(0, 10).replace(/-/g, '');
...
Order.prototype.canTransitionTo = function(newStatus) {
  const validTransitions = {
    ...
    'pending_admin_review': ['pending', 'declined'],
