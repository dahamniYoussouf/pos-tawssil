...
const DeviceToken = sequelize.define(
  "DeviceToken",
  {
    ...
    failure_count: {
      ...
      allowNull: false,
      defaultValue: 0,
      comment: "Consecutive notification delivery failures"
    },
    last_failure_code: {
      type: DataTypes.STRING(120),
      allowNull: true,
      comment: "Last Firebase error code seen for this token"
    },
    last_failure_at: {
      type: DataTypes.DATE,
      allowNull: true,
      comment: "Timestamp of the last notification delivery failure"
    },
    last_seen_at: {
      type: DataTypes.DATE,
      allowNull: true,
  ...
  },
  {
    ...
    indexes: [
      ...
      { fields: ["role", "profile_id", "is_active"] },
