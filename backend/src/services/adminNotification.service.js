...
import { getAdminReviewRequiredCopy } from "./orders/pushCopy.helper.js";
...
const buildOrderReviewPayload = (order, reviewMetadata = {}) => {
  ...
  const orderDetails = {
    ...
    client: {
      id: order.client?.id || order.client_id || null,
      name: clientName,
      phone: order.client?.phone_number || 'Non renseigne',
      address: order.client?.address || order.delivery_address || 'Non renseigne'
    },
    restaurant: {
...
export const createPendingOrderNotification = async (orderId) => {
  try {
    ...
    queueNotifyRole('admin', 'new_notification', {
      id: notification.id,
      type: 'pending_order_timeout',
      message,
...
/**
 * Create admin notification when a client order requires manual review before restaurant routing
 */
export const createOrderReviewRequiredNotification = async (orderId, reviewMetadata = {}) => {
  try {
    const order = await Order.findByPk(orderId, {
      include: [
        {
    ...
    if (!order) {
      console.error(`Order ${orderId} not found for admin review notification`);
      return null;
    }

    if (order.status !== 'pending_admin_review' || !order.requires_admin_review) {
      console.log(`Order ${orderId} no longer requires admin review, skipping notification`);
      return null;
    }
    ...
    const reviewPayload = buildOrderReviewPayload(order, {
      reasons: Array.isArray(reviewMetadata.reasons) ? reviewMetadata.reasons : [],
      client_order_count: reviewMetadata.clientOrderCount ?? reviewMetadata.client_order_count ?? null,
      threshold_amount: reviewMetadata.threshold_amount ?? null,
      threshold_count: reviewMetadata.threshold_count ?? null
    });

    const reasonSummary = Array.isArray(reviewMetadata.reasons) && reviewMetadata.reasons.length > 0
      ? reviewMetadata.reasons.join(', ')
      : 'Vérification requise avant transfert au restaurant';

    const copy = getAdminReviewRequiredCopy({
      orderNumber: order.order_number,
      reasonSummary,
      totalAmount: parseFloat(order.total_amount || 0),
      clientOrderCount: reviewMetadata.clientOrderCount ?? reviewMetadata.client_order_count ?? null
    });

    const notification = await AdminNotification.create({
      order_id: orderId,
      restaurant_id: order.restaurant_id,
      type: 'order_admin_review_required',
      message: copy.message,
      order_details: reviewPayload,
      restaurant_info: restaurantInfo
    });

    console.log(`Admin review notification created: ${notification.id}`);
    ...
    console.error('Error creating admin review notification:', error);
    return null;
  }
};
...
export default {
  ...
  createOrderReviewRequiredNotification,
  getAllNotifications,
  markAsRead,
  resolveNotification
};
