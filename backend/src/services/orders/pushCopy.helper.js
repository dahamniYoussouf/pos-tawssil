...
export const getAdminReviewRequiredCopy = ({
  orderNumber,
  reasonSummary = "",
  totalAmount = null,
  clientOrderCount = null
} = {}) =>
  buildLocalizedNotification({
    titleFr: "🚨 Commande à valider",
    bodyFr: [
      orderNumber ? `Commande #${orderNumber}` : "Nouvelle commande",
      reasonSummary || "Une vérification admin est requise avant l'envoi au restaurant.",
      totalAmount !== null ? `Montant: ${totalAmount} DA` : null,
      clientOrderCount !== null ? `Historique client: ${clientOrderCount} commande(s)` : null
    ].filter(Boolean).join("\n"),
    titleEn: "🚨 Order requires review",
    bodyEn: [
      orderNumber ? `Order #${orderNumber}` : "New order",
      reasonSummary || "Admin verification is required before forwarding to the restaurant.",
      totalAmount !== null ? `Amount: ${totalAmount} DA` : null,
      clientOrderCount !== null ? `Client history: ${clientOrderCount} order(s)` : null
    ].filter(Boolean).join("\n"),
    titleAr: "🚨 Commande à valider",
    bodyAr: [
      orderNumber ? `Commande #${orderNumber}` : "Nouvelle commande",
      reasonSummary || "Une vérification admin est requise avant l'envoi au restaurant.",
      totalAmount !== null ? `Montant: ${totalAmount} DA` : null,
