...
export const updateCashier = async (id, updateData) => {
  return sequelize.transaction(async (transaction) => {
    const cashier = await Cashier.findByPk(id, { transaction });
    if (!cashier) return null;
  
  // Normaliser le numéro de téléphone si présent
    const { email, password, ...cashierUpdates } = updateData || {};

    if (cashierUpdates.phone) {
      cashierUpdates.phone = normalizePhoneNumber(cashierUpdates.phone);
    }

    if (Object.keys(cashierUpdates).length > 0) {
      await cashier.update(cashierUpdates, { transaction });
    }

    if (cashier.user_id && (email !== undefined || password)) {
      const user = await User.findByPk(cashier.user_id, { transaction });
      if (user) {
        const userUpdates = {};
    ...
    return Cashier.findByPk(id, { transaction });
  });
};
