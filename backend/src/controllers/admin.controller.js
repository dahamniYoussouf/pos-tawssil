...
import { getLatestDeviceTokenForProfile, queueNotifyRole } from "../services/notification.service.js";
...
import { restartBackupScheduler } from '../services/backupScheduler.service.js';
...
const defaultOtpRateLimitMax = parsePositiveInt(
...
const SYSTEM_CONFIG_DEFAULTS = [
  ...
  },
  {
    ...
    value: defaultOtpRateLimitMax,
    description: 'Maximum number of OTP requests allowed per 15 minutes'
  },
  {
    key: 'BACKUP_SCHEDULE_ENABLED',
    value: 1,
    description: 'Enable daily database backups (1 = enabled, 0 = disabled)'
  },
  {
    key: 'BACKUP_SCHEDULE_HOUR',
    value: 2,
    description: 'Daily backup hour (0-23)'
  },
  {
    key: 'BACKUP_SCHEDULE_MINUTE',
    value: 0,
    description: 'Daily backup minute (0-59)'
  }
];
...
const normalizeTargetsParam = (value) => {
  if (!value) return [];
  const list = Array.isArray(value) ? value : String(value).split(',');
  return list
    .map((item) => String(item).trim().toLowerCase())
    .filter(Boolean)
    .map((item) => (item.endsWith('s') ? item.slice(0, -1) : item));
};
...
/**
 * GET /admin/warnings/restaurants
 * Liste des restaurants notifiés pour retards (et comportements similaires)
 */
export const getRestaurantWarnings = async (req, res, next) => {
  try {
    const delayTypes = [
    ...
    const rows = await AdminNotification.findAll({
      where: {
        restaurant_id: { [Op.not]: null },
        type: { [Op.in]: delayTypes }
      },
      include: [{
        model: Restaurant,
        as: 'restaurant',
        attributes: ['id', 'name', 'address', 'phone_number', 'status', 'is_active', 'is_premium']
      }],
      attributes: [
        'restaurant_id',
        [Sequelize.fn('COUNT', Sequelize.col('AdminNotification.id')), 'warnings_total'],
        [Sequelize.fn('MAX', Sequelize.col('AdminNotification.created_at')), 'last_warning_at'],
        [Sequelize.literal(`SUM(CASE WHEN "AdminNotification"."is_resolved" = false THEN 1 ELSE 0 END)`), 'warnings_unresolved']
      ],
      group: [
        'AdminNotification.restaurant_id',
        'restaurant.id',
        'restaurant.name',
        'restaurant.address',
      ...
      order: [
        [Sequelize.literal('"warnings_total"'), 'DESC'],
        [Sequelize.literal('"last_warning_at"'), 'DESC']
      ],
      raw: true,
      nest: true
    });
    ...
    const data = rows.map((row) => ({
      ...
      warnings_delay: Number(row.warnings_total) || 0,
      warnings_unresolved: Number(row.warnings_unresolved) || 0,
...
/**
 * GET /admin/warnings/drivers
 * Liste des livreurs notifiés pour retards ou excès d'annulations
 */
export const getDriverWarnings = async (req, res, next) => {
  try {
    const delayTypes = [
    ...
    const allTypes = [...delayTypes, 'driver_excessive_cancellations'];
    const delayTypeList = delayTypes.map((t) => `'${t}'`).join(',');
    ...
    const rows = await AdminNotification.findAll({
      ...
      attributes: [
        'driver_id',
        [Sequelize.fn('COUNT', Sequelize.col('AdminNotification.id')), 'warnings_total'],
        [Sequelize.literal(`SUM(CASE WHEN "AdminNotification"."type" IN (${delayTypeList}) THEN 1 ELSE 0 END)`), 'warnings_delay'],
        [Sequelize.literal(`SUM(CASE WHEN "AdminNotification"."type" = 'driver_excessive_cancellations' THEN 1 ELSE 0 END)`), 'warnings_cancellations'],
        [Sequelize.literal(`SUM(CASE WHEN "AdminNotification"."is_resolved" = false THEN 1 ELSE 0 END)`), 'warnings_unresolved'],
        [Sequelize.fn('MAX', Sequelize.col('AdminNotification.created_at')), 'last_warning_at']
      ],
      group: [
        'AdminNotification.driver_id',
        'driver.id',
        'driver.driver_code',
        'driver.first_name',
        'driver.last_name',
    ...
    const data = rows.map((row) => ({
      ...
      driver: row.driver,
      warnings_total: Number(row.warnings_total) || 0,
      warnings_delay: Number(row.warnings_delay) || 0,
      warnings_cancellations: Number(row.warnings_cancellations) || 0,
      warnings_unresolved: Number(row.warnings_unresolved) || 0,
      last_warning_at: row.last_warning_at
    }));
...
export const updateProfile = async (req, res, next) => {
  try {
    ...
    const { first_name, last_name, email, phone, locale } = req.body;
    ...
    if (locale !== undefined && !SUPPORTED_LOCALES.includes(String(locale).toLowerCase())) {
      return res.status(400).json({
        success: false,
    ...
    if (locale !== undefined) updateData.locale = String(locale).toLowerCase();
...
export const getDeliveryConfig = async (req, res, next) => {
  try {
    ...
    const deliveryFeeBase = await SystemConfig.get('delivery_fee_base', defaultDeliveryFee);
    const deliveryFeePerKm = await SystemConfig.get('delivery_fee_per_km', 0);
...
export const getAllConfigs = async (req, res, next) => {
  try {
    const cacheKey = 'admin:configs:all:v7';
    ...
    mergedConfigs.forEach(config => {
      ...
      if (
        key.includes('max_orders') ||
        key.includes('max_distance') ||
        key.includes('search_radius') ||
        key.includes('delivery_distance') ||
        ...
        ['pending_order_timeout', 'restaurant_preparation_timeout', 'driver_arrival_timeout', 'driver_delivery_timeout'].includes(key)
      ) {
...
export const updateConfig = async (req, res, next) => {
  try {
    ...
    const validations = {
      ...
      'client_order_review_threshold': { min: 1, max: 100, type: 'number' },
      'high_value_order_review_threshold': { min: 0, max: 1000000, type: 'number' },
      'pending_order_timeout': { min: 1, max: 60, type: 'number' },
      'restaurant_preparation_timeout': { min: 1, max: 180, type: 'number' },
      'driver_arrival_timeout': { min: 1, max: 120, type: 'number' },
      'driver_delivery_timeout': { min: 1, max: 240, type: 'number' },
      'default_delivery_fee': { min: 0, max: 10000, type: 'number' },
      'delivery_fee_base': { min: 0, max: 10000, type: 'number' },
      'delivery_fee_per_km': { min: 0, max: 5000, type: 'number' },
      'max_driver_cancellations': { min: 1, max: 20, type: 'number' },
      'driver_nearby_search_radius': { min: 100, max: 50000, type: 'number' },
      'otp_request_use_paid_service': { type: 'boolean' },
      'OTP_RATE_LIMIT_MAX': { min: 1, max: 1000, type: 'number' },
      'BACKUP_SCHEDULE_ENABLED': { min: 0, max: 1, type: 'number' },
      'BACKUP_SCHEDULE_HOUR': { min: 0, max: 23, type: 'number' },
      'BACKUP_SCHEDULE_MINUTE': { min: 0, max: 59, type: 'number' },
    };
    ...
    if (['BACKUP_SCHEDULE_ENABLED', 'BACKUP_SCHEDULE_HOUR', 'BACKUP_SCHEDULE_MINUTE'].includes(key)) {
    ...
    await cacheService.del('admin:configs:all:v5');
    await cacheService.del('admin:configs:all:v6');
...
export const getStatistics = async (req, res, next) => {
  try {
    const cacheKey = 'admin:statistics:v2';
    ...
    const reviewQueueOrders = orders.filter(o =>
      o.status === 'pending_admin_review' && o.requires_admin_review
    );
    const adminReviewStats = {
      total: reviewQueueOrders.length,
      pending: reviewQueueOrders.filter(o => o.admin_review_status === 'pending').length,
      claimed: reviewQueueOrders.filter(o => o.admin_review_status === 'claimed').length,
      low_client_history: reviewQueueOrders.filter(o =>
        Array.isArray(o.admin_review_metadata?.reasons) &&
        o.admin_review_metadata.reasons.includes('low_client_history')
      ).length,
      high_value_order: reviewQueueOrders.filter(o =>
        Array.isArray(o.admin_review_metadata?.reasons) &&
        o.admin_review_metadata.reasons.includes('high_value_order')
      ).length
    };
...
export const getMonitoringSnapshot = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn('pg_stat_statements not available:', error?.message || error);
    }
...
export const notifyAllDrivers = async (req, res, next) => {
  try {
    ...
    const drivers = { length: driverCount };
    ...
    const queueResult = queueNotifyRole('driver', 'admin_broadcast', notificationPayload);
    ...
    res.json({
      success: true,
      message: `Notification queued for ${drivers.length} drivers`,
      data: {
 ...
 * Broadcast a notification to selected audiences (clients, drivers, restaurants)
 */
export const broadcastNotification = async (req, res, next) => {
  try {
    const { message, title, type = 'info', data = {}, targets, i18n } = req.body;
    ...
    const normalizedTargets = normalizeTargetsParam(targets);
    const allowedTargets = new Set(['client', 'driver', 'restaurant', 'admin']);
    const selectedTargets = Array.from(
      new Set(normalizedTargets.filter((target) => allowedTargets.has(target)))
    );
    ...
    const i18nPayload = i18n && typeof i18n === "object" ? i18n : undefined;

    const basePayload = {
      type,
      title: title && String(title).trim().length ? String(title).trim() : 'Admin Notification',
      message: String(message).trim(),
      timestamp: new Date().toISOString(),
      ...(i18nPayload ? { i18n: i18nPayload } : {}),
      ...data
    };
    ...
    for (const target of selectedTargets) {
      if (target === 'client') {
        const count = await Client.count({ where: { is_active: true } });
        counts.clients = count;
        totalRecipients += count;
        queueResults.client = queueNotifyRole('client', 'admin_broadcast', {
          ...basePayload,
          audience: 'clients'
        });
      }
      ...
      if (target === 'driver') {
        const count = await Driver.count({ where: { is_active: true } });
        counts.drivers = count;
        totalRecipients += count;
        queueResults.driver = queueNotifyRole('driver', 'admin_broadcast', {
          ...basePayload,
          audience: 'drivers'
        });
      }
      ...
      if (target === 'restaurant') {
        const count = await Restaurant.count({ where: { is_active: true } });
        counts.restaurants = count;
        totalRecipients += count;
        queueResults.restaurant = queueNotifyRole('restaurant', 'admin_broadcast', {
          ...basePayload,
      ...
      if (target === 'admin') {
        const count = await Admin.count({ where: { is_active: true } });
        counts.admins = count;
        totalRecipients += count;
        queueResults.admin = queueNotifyRole('admin', 'admin_broadcast', {
          ...basePayload,
          audience: 'admins'
        });
      }
    ...
    const queueDropped = Object.values(queueResults).some(
      (result) => result && !result.accepted
    );

    res.status(queueDropped ? 503 : 200).json({
      success: !queueDropped,
      message: queueDropped
        ? "One or more notifications could not be queued"
