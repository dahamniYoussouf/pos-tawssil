...
import { computeRecommendedDishPricing } from "../services/homepagePricing.helper.js";
...
const buildRestaurantMetaMap = (restaurants = []) => {
  ...
  restaurants.forEach((restaurant) => {
    ...
    map.set(String(restaurantId), {
      ...
      delivery_fee: toOptionalNumber(restaurant.delivery_fee),
...
const enrichRecommendedDishes = (recommendedDishes, restaurantMetaMap, promotions = []) => {
  ...
  return recommendedDishes.map((dish) => {
    ...
    const pricing = computeRecommendedDishPricing(dish, promotions);
    ...
    return {
      ...
      delivery_fee: restaurantMeta?.delivery_fee ?? null,
      ...
      is_on_promotion: pricing.is_on_promotion
    };
  });
};
