...
export const getMyMenuItems = async (req, res, next) => {
  try {
    ...
    const order = [];
    if (!category_id) {
      order.push([{ model: FoodCategory, as: 'category' }, 'ordre_affichage', 'ASC']);
      order.push([{ model: FoodCategory, as: 'category' }, 'nom', 'ASC']);
    }
...
export const getCashierAvailableMenuItems = async (req, res, next) => {
  try {
      ...
      include: [
        {
          model: FoodCategory,
          as: 'category',
          attributes: ['id', 'nom', 'ordre_affichage']
        },
        {
          model: Addition,
          as: 'additions',
    ...
    rows.forEach((item) => {
      ...
      const category = plainItem.category;
      const currentCategoryId = category?.id || plainItem.category_id;
      if (!currentCategoryId) return;

      let group = categoryMap.get(currentCategoryId);
      if (!group) {
        group = {
          id: currentCategoryId,
          nom: category?.nom || null,
          ordre_affichage: category?.ordre_affichage ?? null,
          items: []
        };
        categoryMap.set(currentCategoryId, group);
        groupedCategories.push(group);
      }

      const { category: _category, ...itemPayload } = plainItem;
...
export const getCashierMenuItems = async (req, res, next) => {
  try {
    ...
    rows.forEach((item) => {
      const plainItem = item.get({ plain: true });
      const category = plainItem.category;
      const categoryId = category?.id || plainItem.category_id;
      if (!categoryId) return;

      let group = categoryMap.get(categoryId);
      if (!group) {
        group = {
          id: categoryId,
          nom: category?.nom || null,
