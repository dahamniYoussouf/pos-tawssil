...
DO 'BEGIN CREATE TYPE "public"."enum_restaurants_availability_status" AS ENUM(''open'', ''closed'', ''vacation'', ''saturated'', ''other''); EXCEPTION WHEN duplicate_object THEN null; END';

CREATE TABLE IF NOT EXISTS "restaurants" ("id" UUID NOT NULL  , "user_id" UUID NOT NULL UNIQUE REFERENCES "users" ("id") ON DELETE CASCADE ON UPDATE CASCADE , "name" VARCHAR(255) NOT NULL, "description" TEXT, "address" TEXT, "phone_number" VARCHAR(255) , "email" VARCHAR(255) , "location" GEOGRAPHY(POINT,4326) NOT NULL, "rating" DECIMAL(2,1) DEFAULT 0 , "image_url" VARCHAR(255) , "is_active" BOOLEAN NOT NULL DEFAULT true , "is_premium" BOOLEAN NOT NULL DEFAULT false , "status" "public"."enum_restaurants_status" DEFAULT 'pending', "availability_status" "public"."enum_restaurants_availability_status" NOT NULL DEFAULT 'open', "availability_note" TEXT , "opening_hours" JSON , "created_at" TIMESTAMP WITH TIME ZONE NOT NULL, "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL, PRIMARY KEY ("id")); COMMENT ON COLUMN "restaurants"."id" IS 'Primary key (UUID)'; COMMENT ON COLUMN "restaurants"."user_id" IS 'Reference to user account'; COMMENT ON COLUMN "restaurants"."phone_number" IS 'Restaurant contact phone number'; COMMENT ON COLUMN "restaurants"."email" IS 'Restaurant contact email'; COMMENT ON COLUMN "restaurants"."rating" IS 'Note sur 5 étoiles'; COMMENT ON COLUMN "restaurants"."image_url" IS 'URL de l''image principale du restaurant'; COMMENT ON COLUMN "restaurants"."is_active" IS 'Restaurant actif ou non'; COMMENT ON COLUMN "restaurants"."is_premium" IS 'Restaurant premium ou non'; COMMENT ON COLUMN "restaurants"."opening_hours" IS 'Opening hours per day, e.g.: { Mon: {open: 9:00 a.m., close: 6:00 p.m.}, Tue: {...} }';

CREATE TABLE IF NOT EXISTS "restaurant_printers" ("id" UUID PRIMARY KEY, "restaurant_id" UUID NOT NULL REFERENCES "restaurants" ("id") ON DELETE CASCADE ON UPDATE CASCADE, "name" VARCHAR(255) NOT NULL, "type" VARCHAR(50) NOT NULL DEFAULT 'general', "ip" VARCHAR(45) NOT NULL, "port" INTEGER NOT NULL DEFAULT 9100, "is_enabled" BOOLEAN NOT NULL DEFAULT true, "paper_width_mm" INTEGER NOT NULL DEFAULT 80, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS "option_groups" ("id" UUID , "menu_item_id" UUID NOT NULL REFERENCES "menu_items" ("id") ON DELETE CASCADE ON UPDATE CASCADE, "nom" VARCHAR(255) NOT NULL, "description" TEXT, "is_required" BOOLEAN DEFAULT false, "ordre_affichage" INTEGER DEFAULT 0, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL, "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL, PRIMARY KEY ("id"));

CREATE TABLE IF NOT EXISTS "additions" ("id" UUID , "menu_item_id" UUID NOT NULL REFERENCES "menu_items" ("id") ON DELETE CASCADE ON UPDATE CASCADE, "option_group_id" UUID REFERENCES "option_groups" ("id") ON DELETE SET NULL ON UPDATE CASCADE, "nom" VARCHAR(255) NOT NULL, "description" TEXT, "prix" DECIMAL(10,2) NOT NULL, "is_available" BOOLEAN DEFAULT true, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL, "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL, PRIMARY KEY ("id"));
...
DO 'BEGIN CREATE TYPE "public"."enum_admin_notifications_type" AS ENUM(''pending_order_timeout'', ''restaurant_preparation_timeout'', ''driver_arrival_timeout'', ''restaurant_unresponsive'', ''driver_unresponsive'', ''driver_excessive_cancellations''); EXCEPTION WHEN duplicate_object THEN null; END';
...
CREATE INDEX IF NOT EXISTS "restaurant_printers_restaurant_id" ON "restaurant_printers" ("restaurant_id");
...
CREATE INDEX IF NOT EXISTS "option_groups_menu_item_id" ON "option_groups" ("menu_item_id");

CREATE INDEX IF NOT EXISTS "option_groups_menu_item_id_is_required" ON "option_groups" ("menu_item_id", "is_required");

CREATE INDEX IF NOT EXISTS "additions_menu_item_id" ON "additions" ("menu_item_id");

CREATE INDEX IF NOT EXISTS "additions_option_group_id" ON "additions" ("option_group_id");
