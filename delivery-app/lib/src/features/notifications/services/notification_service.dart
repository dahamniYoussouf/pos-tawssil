...
class NotificationService {
  ...
  static const String _channelId = 'tawsil_delivery_notifications';
  static const String _channelName = 'Delivery Notifications';
  static const String _channelDescription =
      'Real-time order and driver notifications';
  ...
  void _handleRemoteMessage(
    ...
    dev.log('[NotificationService] Handling FCM event: type=$eventType, data=$data',
        name: 'NotificationService');
  ...
  Future<void> _registerDeviceToken(String fcmToken) async {
    ...
    final response = await _baseApiService.postRequest(
      ...
      data: <String, dynamic>{
        'token': fcmToken,
        'platform': platform,
        'device_id': '$platform-driver-$userId',
      },
    );
  ...
  String _resolveEventType(Map<String, dynamic> data) {
    ...
    if (rawType == 'max_orders_updated') {
      data['type'] = rawType;
      return 'config_update';
    }
    if (rawType == 'order_assigned' ||
        rawType == 'delivery_complete' ||
        rawType == 'order_location') {
      data['type'] = rawType;
      return 'notification';
    }
    return _defaultEventType;
